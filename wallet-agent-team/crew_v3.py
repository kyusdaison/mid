...
web3_infra_agent = Agent(
    ...
    backstory="""区块链工程师，专精于 EVM 钱包和 viem TypeScript 集成 (Gemini 3.1 Pro 驱动)。
你的专长：
...
local_db_agent = Agent(
    ...
    backstory="""专家级 Zustand 状态管理和加密移动存储工程师 (Gemini 3.1 Pro 驱动)。
你的实现：
...
edge_case_specialist = Agent(
    ...
    backstory="""专精于破坏事物的工程师——在用户之前发现问题 (Gemini 3.1 Pro 驱动)。
每个边界情况都需要设计好的 UI 响应：
1. 相机权限被拒绝
2. 网络离线
3. 凭证已过期
...
rn_ux_motion_agent = Agent(
    ...
    backstory="""动效设计师，坚信每个动画都必须服务于理解，不是表演 (Gemini 3.1 Pro 驱动)。
规则：
1. 动画时长 200-400ms
2. 不使用循环装饰性动画
...
qa_destroyer_agent = Agent(
    ...
    backstory="""QA 工程师，使命是：如果它能坏，必须在测试中先坏 (Gemini 3.1 Pro 驱动)。
测试覆盖：
1. 正常流程
2. 边界条件
3. 错误状态
...
devops_agent = Agent(
    role="DevOps工程师",
    goal="配置 CI/CD、Docker 部署和自动化构建流程",
    backstory="""DevOps 专家 (Gemini 3.1 Pro 驱动)，专精于 React Native 项目的自动化部署。
你的职责：
1. 配置 GitHub Actions CI/CD
2. 编写 Dockerfile
3. 设置 EAS (Expo Application Services) 构建
4. 自动化测试和发布流程""",
    tools=[file_read, file_write, dir_read],
    llm=llm_engineering,
    verbose=True,
...
doc_writer_agent = Agent(
    role="技术文档师",
    goal="编写 API 文档、README 和开发者指南",
    backstory="""技术写作专家 (GPT-5.4 驱动)，擅长将复杂技术转化为清晰文档。
你的产出：
1. API 接口文档
2. 组件使用说明
3. 开发者 onboarding 指南
...
localization_agent = Agent(
    role="国际化专家",
    goal="实现多语言支持和本地化",
    backstory="""国际化工程师 (Gemini 3.1 Pro 驱动)，精通 React Native i18n。
你的任务：
1. 配置 i18next 多语言框架
2. 提取和翻译文案
3. 支持 RTL 语言
...
# --- 管理层（GPT-5.4）---
# 注意：分层流程的 manager_agent 不能有 tools！
manager_agent = Agent(
    role="项目经理",
    goal="协调各 Agent 工作，确保项目按时交付",
    backstory="""资深项目经理（GPT-5.4 驱动），擅长协调 AI Agent 团队。
你的职责：
1. 分解任务并分配给合适的 Agent
2. 监控进度和质量
3. 解决 Agent 之间的冲突
4. 汇总输出并生成最终报告""",
    # Manager agent 不能有 tools！
    tools=[],
    llm=llm_strategy,
    verbose=True,
    allow_delegation=True  # 唯一可以委派任务的 Agent
)
...
先读取当前文件，然后写入升级版本。""",
        ...
        # human_input 已移除，全自动运行
    )

    task_web3_integration = Task(
        description="""实现与 FCDIDRegistry 合约的交互功能：
- 读取合约验证凭证状态
- 处理合约调用错误
- 优化 gas 使用

输出：更新的 services/fcChainService.ts""",
        expected_output="更新后的 fcChainService.ts",
        agent=web3_infra_agent
    )
...
输出：审计报告及严重程度评级。""",
        ...
        llm=llm_deep_reasoning  # 使用深度推理
    )

    # Phase 6: 扩展功能
    task_localization = Task(
        description="""为 VerifyScreen 添加国际化支持：
- 提取所有 UI 文本到 i18n 文件
- 支持英文、中文、西班牙文
- 确保 RTL 语言兼容

输出：更新后的 i18n 配置和翻译文件。""",
        expected_output="国际化实现",
        agent=localization_agent
    )

    task_docs = Task(
        description="""编写 VerifyScreen 的技术文档：
- 组件 API 文档
- 使用示例
- 架构说明
...
def run_crew_v3(mission_type: str = "verify_screen"):
    """启动 CrewAI V3 团队（分层流程）"""
    ...
    crew = Crew(
        agents=agents,
        tasks=tasks,
        process=Process.hierarchical,  # 分层流程！
        manager_agent=manager_agent,    # 项目经理协调
        verbose=True,
        memory=True,
        embedder={
            "provider": "openai",
    ...
    print("\n" + "="*70)
    print("🚀 MID WALLET - CREWAI V3 特工团队启动")
    print("="*70)
    print(f"配置：14 Agent + 分层流程 + 人类反馈")
    print(f"战略层：GPT-5.4 × 5 Agent")
    print(f"工程层：Gemini 3.1 Pro × 6 Agent")
    print(f"管理层：GPT-5.4 × 1 Agent（项目经理）")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="MID Wallet CrewAI V3 团队")
    parser.add_argument(
        "--mission",
